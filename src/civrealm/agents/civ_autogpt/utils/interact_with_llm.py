...
tmp_dia = [{'role': 'user', 'content': 'Hello, Who are you?'}, {'role': 'assistant', 'content': 'I am LLM.'}, {
    'role': 'user', 'content': 'Good, Give me an example about how to use you.'}]
...
def send_message_to_vicuna(dialogue: list = tmp_dia, config=tmp_config):
    ...
    response = matches.split(
        'json')[-1].split('</s>')[0].strip().replace('\n', '').replace('\r', '')
