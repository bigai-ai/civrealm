...
from civrealm.freeciv.map.map_const import TERRAIN_NAMES, EXTRA_NAMES, DIR8_NAMES
from civrealm.freeciv.utils.unit_improvement_const import UNIT_TYPES
from civrealm.agents.civ_autogpt.GPTAgent import GPTAgent
            ...
            if ctrl_type == 'unit':
                ...
                while exec_action_name is None:
                    ...
                    exec_action_name = self.gpt_agent.process_command(
                        response, obs_input_prompt, current_unit_name, current_avail_actions_list)
                ...
                valid_city_id, current_city_name, valid_city_actions_list = self.get_next_valid_actor(
                    observations, info, city_dict, ctrl_type)
                ...
                while exec_action_name is None:
                    ...
                    self.gpt_agent.memory.save_context({'user': obs_input_prompt}, {'assistant': str(response)})
                    exec_action_name = self.gpt_agent.process_command(
                        response, obs_input_prompt, current_city_name, current_city_avail_actions_list)
