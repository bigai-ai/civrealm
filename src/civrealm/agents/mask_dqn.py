...
class DQN:
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon, target_update, device):
        ...
        self.target_q_net = Qnet(
            state_dim, hidden_dim, self.action_dim).to(device)
    ...
    def take_action(self, state, action_mask):
        if np.random.random() < self.epsilon:
            ...
            action_mask = torch.tensor(
                [action_mask], dtype=torch.float).to(self.device)
    ...
    def update(self, transition_dict):
        ...
        next_states = torch.tensor(transition_dict['next_states'].astype(
            np.int64), dtype=torch.float).to(self.device)
        dones = torch.tensor(
            transition_dict['dones'], dtype=torch.float).view(-1, 1).to(self.device)
        action_mask = torch.tensor(
            transition_dict['masks'], dtype=torch.float).to(self.device)

        q_values = self.q_net(states, action_mask).gather(1, actions)
        max_next_q_values = self.target_q_net(
            next_states, action_mask).max(1)[0].view(-1, 1)
...
class DQNAgent(BaseAgent):
    def __init__(self):
        ...
        self.unit_agent = DQN(state_dim, hidden_dim, action_dim,
    ...
    def make_mask(self, valid_action_dict):
        action_mask = [
            0 if action in valid_action_dict else 1 for action in self.unit_action_dict]
        return action_mask
    ...
    def act(self, observations, obs, info):
        ...
        for ctrl_type in available_actions.keys():
            ...
            if self.action_ctrl == 'unit':
                ...
                action_name = self.unit_agent.take_action(
                    obs, self.action_mask)
                self.action_choose = action_name  # save policy output to replybuffer

                print('action_name: ', list(
                    self.unit_action_dict.values())[action_name])
