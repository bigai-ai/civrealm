...
class DiplOptions(ActionList):
    def __init__(self, ws_client, rule_ctrl, city_ctrl, player_ctrl, diplomacy_clause_map, contact_turns_left, reason_to_cancel, diplstates, others_diplstates):
    ...
    def update_counterpart_options(self, counter_id, cur_player, counterpart, new_city_set):
        if self.diplomacy_possible(cur_player, counterpart):
            ...
            self.add_action(counter_id, StopNegotiate(self.diplomacy_clause_map, self.contact_turns_left, cur_player, counterpart))

            self.add_action(counter_id, AcceptTreaty(self.diplomacy_clause_map, self.contact_turns_left, cur_player, counterpart))

            self.add_action(counter_id, CancelTreaty(self.diplomacy_clause_map, self.contact_turns_left, cur_player, counterpart, self.city_ctrl, self.rule_ctrl, self.reason_to_cancel, self.diplstates))

            self.add_action(counter_id, CancelVision(self.diplomacy_clause_map, self.contact_turns_left, cur_player, counterpart))
    ...
    def update_clause_options(self, counter_id, cur_player, counterpart, new_city_set):
        for ctype in BASE_CLAUSES:
            ...
            self.add_action(counter_id, AddClause(ctype, 1, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left, self.ws_client, self.diplstates, self.others_diplstates))

            self.add_action(counter_id, RemoveClause(ctype, 1, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left))
        ...
        if self.rule_ctrl.game_info["trading_tech"]:
            for tech_id in self.rule_ctrl.techs:
                ...
                self.add_action(counter_id, AddTradeTechClause(clause_type, tech_id, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left, self.ws_client, self.diplstates, self.others_diplstates))

                self.add_action(counter_id, RemoveClause(clause_type, tech_id, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left))
    ...
    def update_trade_gold_clauses(self, counter_id, cur_player, counterpart):
        for pgold in range(self.before_max_gold + 1, self.current_max_gold + 1, GOLD_STEP):
            self.add_action(counter_id, AddTradeGoldClause(player_const.CLAUSE_GOLD, pgold, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left, self.ws_client, self.diplstates, self.others_diplstates, self.players))

            self.add_action(counter_id, RemoveClause(
                player_const.CLAUSE_GOLD, pgold, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left))
    ...
    def update_trade_city_clauses(self, counter_id, cur_player, counterpart, new_city_set):
        for pcity in new_city_set:
            self.add_action(counter_id, AddTradeCityClause(player_const.CLAUSE_CITY, pcity, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left, self.ws_client, self.diplstates, self.others_diplstates, self.city_ctrl))

            self.add_action(counter_id, RemoveClause(
                player_const.CLAUSE_CITY, pcity, counter_id, cur_player, counterpart, self.diplomacy_clause_map, self.contact_turns_left))
...
class CancelTreaty(StartNegotiate):
    ...
    def __init__(self, diplomacy_clause_map, contact_turns_left, cur_player, counterpart, city_ctrl, rule_ctrl, reason_to_cancel, diplstates):
...
class RemoveClause(base_action.Action):
    ...
    def can_start_a_meeting(self):
        if self.cur_player['playerno'] == self.counter_id:
            self.start_meeting = StartNegotiate(self.diplomacy_clause_map, self.contact_turns_left, self.counterpart, self.cur_player)
        else:
            self.start_meeting = StartNegotiate(self.diplomacy_clause_map, self.contact_turns_left, self.cur_player, self.counterpart)
...
class AddClause(RemoveClause):
    ...
    def __init__(self, clause_type, value, counter_id, cur_player, counterpart, diplomacy_clause_map, contact_turns_left, ws_client, diplstates, others_diplstates):
...
class AddTradeGoldClause(AddClause):
    ...
    def __init__(self, clause_type, value, counter_id, cur_player, counterpart, diplomacy_clause_map, contact_turns_left, ws_client, diplstates, others_diplstates, players):
        super().__init__(clause_type, value, counter_id, cur_player, counterpart, diplomacy_clause_map, contact_turns_left, ws_client, diplstates, others_diplstates)
...
class AddTradeCityClause(AddClause):
    ...
    def __init__(self, clause_type, value, counter_id, cur_player, counterpart, diplomacy_clause_map, contact_turns_left, ws_client, diplstates, others_diplstates, city_ctrl):
        super().__init__(clause_type, value, counter_id, cur_player, counterpart, diplomacy_clause_map, contact_turns_left, ws_client, diplstates, others_diplstates)
