...
def controller():
    ...
    controller.set_parameter(
        'debug.load_game', 'testcontroller_T169_2023-07-26-09_04')
    yield controller
...
def test_dipl_stop_negotiate(controller):
    fc_logger.info("test_dipl_stop_negotiate")
    _, options = get_first_observation_option(controller)

    player_opt = options['dipl']
    stop_negotiate_act = find_keys_with_keyword(
        player_opt.get_actions(4, valid_only=True), 'stop_negotiation')[0]
